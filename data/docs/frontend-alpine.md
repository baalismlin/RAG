...
```html
<div x-data="{}" x-init="console.log('Component ready')"></div>
```
...
```html
<img x-bind:src="imageUrl" /> <button :disabled="!isValid">Submit</button>
```
...
```html
<input x-ref="inputField" /> <button @click="$refs.inputField.focus()">Focus</button>
```
