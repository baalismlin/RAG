...
const SYMBOL_CAPTURE_NAMES = new Set(["class", "interface", "function", "method", "type_alias", "arrow_fn"]);
...
function getModifiers(node: Parser.SyntaxNode): string[] {
  ...
  if (node.parent?.type === "export_statement" && node.parent.text.startsWith("export default")) mods.push("default");
...
function buildSymbol(captureName: CaptureName, node: Parser.SyntaxNode, nameNode: Parser.SyntaxNode | null): SymbolInfo | null {
