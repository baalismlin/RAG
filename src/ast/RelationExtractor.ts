...
function symbolId(filePath: string, name: string, type: string, startLine: number): string {
  return createHash("sha256").update(`${filePath}::${name}::${type}::${startLine}`).digest("hex").slice(0, 32);
}
...
export class RelationExtractor {
  extract(content: string, filePath: string): { symbols: CodeSymbol[]; relations: SymbolRelation[] } {
    const language = this.detectLanguage(filePath);
    const rawSymbols = language === "python"
      ? this.extractPythonSymbols(content)
      : this.extractJsTsSymbols(content);
    ...
    const relations: SymbolRelation[] = language === "python"
  ...
  private extractJsTsSymbols(content: string): RawSymbol[] {
    ...
    const patterns: Array<{ regex: RegExp; type: SymbolKind; nameGroup: number }> = [
      { regex: /^export\s+(?:default\s+)?(?:abstract\s+)?class\s+(\w+)/, type: "class", nameGroup: 1 },
      ...
      { regex: /^export\s+(?:default\s+)?(?:async\s+)?function\s*\*?\s*(\w+)/, type: "function", nameGroup: 1 },
      { regex: /^(?:async\s+)?function\s*\*?\s*(\w+)/, type: "function", nameGroup: 1 },
      { regex: /^export\s+(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?\(/, type: "function", nameGroup: 1 },
  ...
  private extractPythonSymbols(content: string): RawSymbol[] {
    ...
    while (i < lines.length) {
      ...
      if (classMatch ?? funcMatch) {
        ...
        symbols.push({ name, type, startLine, endLine, content: lines.slice(i, endLine).join("\n"), signature: line.trim() });
        i = endLine;
      } else {
        i++;
      }
  ...
  private extractJsTsRelations(content: string, filePath: string, symbols: CodeSymbol[]): SymbolRelation[] {
    ...
    lines.forEach((line, idx) => {
      ...
      if (importMatch) {
        const names = importMatch[1]
          ? importMatch[1].split(",").map((n) => n.trim().replace(/\s+as\s+\w+$/, "")).filter(Boolean)
          : [importMatch[2]];
      ...
      const extendsMatch = trimmed.match(/^(?:export\s+)?(?:abstract\s+)?class\s+(\w+)\s+extends\s+(\w+)/);
      if (extendsMatch) {
      ...
      const implementsMatch = trimmed.match(/^(?:export\s+)?(?:abstract\s+)?class\s+(\w+).*implements\s+(.+?)(?:\s*\{|$)/);
      if (implementsMatch) {
        const child = symbolByName.get(implementsMatch[1]);
        if (child) {
          const ifaces = implementsMatch[2].split(",").map((n) => n.trim()).filter(Boolean);
  ...
  private extractPythonRelations(content: string, filePath: string, symbols: CodeSymbol[]): SymbolRelation[] {
    const relations: SymbolRelation[] = [];
    const lines = content.split("\n");
    const symbolByName = new Map(symbols.map((s) => [s.name, s]));
    ...
    lines.forEach((line, idx) => {
      ...
      if (classMatch) {
        ...
        if (child) {
          const bases = classMatch[2].split(",").map((n) => n.trim()).filter((n) => n !== "object");
