...
export class GraphStore implements IGraphStore {
  ...
  async upsertRelations(relations: SymbolRelation[]): Promise<void> {
    ...
    for (const r of relations) {
      await pool.query(
        ...
        [r.fromSymbolId, r.toSymbolName, r.toSymbolId ?? null, r.relationType, r.filePath, r.lineNumber ?? null]
      );
    }
  }
  ...
  async traverse(startId: string, direction: "outgoing" | "incoming", maxDepth: number): Promise<TraversalNode[]> {
    const visited = new Set<string>([startId]);
    const result: TraversalNode[] = [];
    const queue: Array<{ id: string; depth: number; via?: SymbolRelation }> = [{ id: startId, depth: 0 }];
